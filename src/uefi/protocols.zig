...
const CompnentName2Protocol = @import("component_name_protocol.zig").ComponentName2Protocol;
...
fn dumpUefiHandleProtocols(bs: *uefi.tables.BootServices) void {
    ...
    _ = bs.locateHandleBuffer(
        uefi.tables.LocateSearchType.AllHandles,
        null, // *Protocol
        null, // *SearchKey
        &handle_count,
        &handles,
    ...
    io.println("", .{});
    io.println("UEFI Handles = {}", .{handle_count});
    ...
    while (i < handle_count) : (i += 1) {
        ...
        io.println("Protocols = {}", .{proto_count});

        var j: usize = 0;
        while (j < proto_count) : (j += 1) {
            io.print("  ", .{});
            io.printGuid(proto_guids[j].*);
            io.print("   ", .{});
